...
private final class ClientConnection {
    ...
    ///
    /// IMPORTANT: we use `[UInt8]` rather than `Data` because Swift `Data`
    /// can be backed by a bridged `NSData` with a non-zero startIndex after
    /// slicing operations (even `Data(slice)` can retain the underlying
    /// reference).  `[UInt8]` is guaranteed 0-based and has no bridging
    /// surprises, eliminating the subscript-out-of-bounds crash that occurred
    /// after the second file transfer.
    ...
    private func receive() {
        connection.receive(minimumIncompleteLength: 1, maximumLength: 65536) { [weak self] data, _, isComplete, error in
            ...
            if let data = data, !data.isEmpty {
                // Append bytes to our flat byte buffer.  Data → [UInt8]
                // copies the bytes, so we never share storage with any
                // NSData instance returned by Network.framework.
    ...
    /// Works on a plain `[UInt8]` so indices are always 0-based and valid.
    private func drainBuffer() {
        while !buffer.isEmpty {
            if fileBytesRemaining > 0 {
                ...
                let chunkBytes = Array(buffer.prefix(take))
                buffer.removeFirst(take)
                fileBytesRemaining -= take
                onFileData?(Data(chunkBytes))
                ...
                buffer.removeSubrange(0...idx)   // remove line + '\n'
